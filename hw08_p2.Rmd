...
map_dfr(genres_of_interest, genre_count) %>%
  ...
  geom_col() +
  labs(title = "Common genres in top 75 mangas", x = "Genre", y = "Count", fill = "") +
  geom_text()
...
shoujo_genre_count <- function(genre_string){
  ...
  manga_75 %>%
    ...
    filter(str_detect(genre, genre_string)) %>%
    count() %>%
    mutate(genre = genre_string, percent = n/length_shoujo)
  }

map_dfr(genres_of_interest, shoujo_genre_count)

length_shoujo <- manga_75 %>%
  filter(str_detect(genre, "Shoujo")) %>%
  count() %>%
  pull()
...
manga_75 %>%
    ...
    count() %>%
    mutate(genre = "Romance", percent = n/length_shoujo)
