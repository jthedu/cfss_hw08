...
#manga_p1_df %>%
 # mutate(row = row_number()) %>%
  #pivot_wider(names_from = row, values_from = date, names_prefix = 'event')
...
left_join(almost_manga_top, almost_manga_bottom, by = "rank") %>%
  select(-c(row.x, genre.x, title.y, row.y)) %>%
  rename(title = title.x, genre = genre.y)



#manga_table_nodes <- html_nodes(manga_p1, css = "table")
...
manga_box_nodes <- html_nodes(manga_p1, "#main_content .text.text-center , .text.text-center b, .col-md-10")
